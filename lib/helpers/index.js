...
const createCsvFile = (data, status) => {
  const output = `resultat-${new Date().toLocaleDateString()}.csv`;
...
const createCsvRow = (data) => {
  return data.map((row) => {
    return {
      Client: row.name,
      "Premier Gestionnaire": _.isEmpty(row.gestionnaires[0])
        ? ""
        : row.gestionnaires[0].name,
      "Deuxieme Gestionnaire": _.isEmpty(row.gestionnaires[1])
        ? ""
        : row.gestionnaires[1].name,
    };
  });
};
